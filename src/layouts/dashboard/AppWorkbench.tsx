...
export default function AppWorkbench() {
    ...
    const tooltipText = open ? "Close Easy" : "Open Easy"
    ...
    return (
        <Drawer.Root open={open} onOpenChange={handleOpenChange} direction="bottom">
            <Tooltip>
                <TooltipTrigger asChild>
                    <button
                        ...
                        className={cn(
                            "fixed right-6 z-50 flex size-14 items-center justify-center rounded-full border border-mauve-200 bg-background text-white shadow-md shadow-black/10 outline-none transition-all duration-200 hover:scale-105 focus-visible:ring-2 focus-visible:ring-neutral-950 focus-visible:ring-offset-2 dark:bg-neutral-700 dark:text-neutral-950 dark:focus-visible:ring-white bottom-6",
                            // open
            ...
            <Drawer.Portal>
                ...
                <Drawer.Content
                    className="right-1.5 bottom-1.5 fixed z-60 flex h-[calc(100%-var(--navbar-height))] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] outline-none sm:w-120 lg:w-1/3"
