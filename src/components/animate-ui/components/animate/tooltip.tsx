...
function TooltipContent({
  ...
  return (
    <TooltipContentPrimitive
      ...
      {...props}
    >
      <motion.div className="overflow-hidden px-3 py-1.5 text-[0.8125rem] text-balance">
        <motion.div layout={layout}>{children}</motion.div>
      </motion.div>
      <TooltipArrowPrimitive
        data-slot="tooltip-arrow"
        className="fill-neutral-950/90 size-3 data-[side='bottom']:translate-y-[1px] data-[side='right']:translate-x-[1px] data-[side='left']:translate-x-[-1px] data-[side='top']:translate-y-[-1px]"
        tipRadius={2}
      />
    </TooltipContentPrimitive>
  );
