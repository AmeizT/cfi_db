...
import {
  ReportStatusBadge,
  reportStatusLabel,
} from "@/features/reports/workflow/components/ReportStatusBadge";
...
import type {
  ReportSectionStatus,
  WorkflowReport,
} from "@/features/reports/workflow/types";
...
type MonthlyReportYearProgressProps = {
  year: number;
...
const segmentStyles: Record<VisualSectionStatus, string> = {
...
function reviewStatus(report: WorkflowReport): VisualSectionStatus {
  ...
  if (report.status === "ready_to_submit" || report.capabilities.can_submit) return "in_progress";
  return "not_started";
}

function configuredSections(report: WorkflowReport) {
  return REPORT_WIZARD_SECTIONS.map((section) => {
    if (section.id === "review") return { meta: section, status: reviewStatus(report) };
    const workflowSection = report.sections.find(
      (item) => item.key === section.backendId || item.name === section.backendId,
    );
    return { meta: section, status: workflowSection?.status ?? "not_started" };
  });
}
...
function reportSummary(report: WorkflowReport) {
  ...
  const completed = sections.filter((item) =>
    item.status === "completed" || item.status === "submitted" || item.status === "no_activity",
  ).length;
  ...
  return (
    ...
    >
      {sections.map((item) => (
        <span
          key={item.meta.id}
          className={cn("h-1.5 rounded-full", segmentStyles[item.status])}
          aria-hidden="true"
        />
      ))}
    </div>
...
export function MonthlyReportYearProgress({
  year,
  activeReportId,
  activePeriodStart,
  method,
  uploadType,
  amendmentContext,
}: MonthlyReportYearProgressProps) {
      ...
      <SheetContent side="right" className="w-[min(32rem,96vw)] gap-0 p-0 sm:max-w-none">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-300">
          {query.isLoading ? (
            ...
            <Accordion type="single" collapsible defaultValue={activeReport?.period_start} className="grid gap-2">
              {reports.map((report) => {
                    ...
                    <AccordionTrigger className="py-3 hover:no-underline">
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2">
                          ...
                          {current ? (
                            <Badge variant="secondary" className="bg-primary/10 text-[0.625rem] text-primary">Current</Badge>
                          ) : null}
                        </span>
                        ...
                        {configuredSections(report).map((item) => {
                          ...
                          return report.id ? (
                            <SheetClose asChild key={item.meta.id}>
                              <Link
                                href={createMonthlyReportHref(item.meta.id, context)}
                      ...
                      {report.id ? (
                        <SheetClose asChild>
                          <Button asChild size="sm" className="mt-3 w-full">
                            <Link href={createMonthlyReportHref(getMonthlyReportResumeSection(report), context)}>Open report</Link>
                          </Button>
                        </SheetClose>
                      ) : (
