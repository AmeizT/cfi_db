...
export function AssemblyLeaderboard({ assemblies, onSelect }: Props) {
  ...
  const filtered = active === "all"
    ? assemblies
    : assemblies.filter((a) => a.status === "Open");
      ...
      <div className="bg-white border border-gray-100 rounded-xl divide-y divide-gray-50">
        {filtered.map((a) => (
          ...
          >
            ...
            <ProgressBar pct={0} size="sm" />
            <StatusPill status={a.status as ComplianceStatus} className="hidden sm:inline-flex" />
          </div>
        ))}
        {filtered.length === 0 && (
